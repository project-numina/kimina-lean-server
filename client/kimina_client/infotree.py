import re
from typing import Optional, Dict, List, Tuple, Any, TypedDict, TypeAlias
...
class InfotreeItem(TypedDict, total=False):
    node: NodeInfo
    kind: str
    children: List["InfotreeItem"]

Infotree: TypeAlias = List[InfotreeItem]
...
def extract_nodes_and_edges(
    ...
    ----------
    infotree : Infotree,
        A list of InfotreeItems each containing 'node' and 'children'.
...
def _flatten_chain(nodes: Dict[int, NodeInfo], edges: List[Tuple[int, int, Dict[str, Any]]], parent_id: int, child_id: int) -> None:
    """
...
def get_intervals(nodes: Dict[int, NodeInfo]) -> List[Dict[str, Any]]:
    """
...
def adjust_intervals(intervals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    ...
    Returns
    -------
    intervals_sorted : List[Dict[str, Any]]
...
def retrieve_tactics(intervals: List[Dict[str, Any]], source_lines: list[str]) -> list[Dict[str, Any]]:
    """
...
def separate_trailing_comment(s: str) -> Tuple[str, str]:
    ...
    trailing_comments : List[str] = []  # This will contain all the trailing comments
    ...
    if trailing_comments == []:
        ...
        return clean_code, comment


def transfer_trailing_whitespaces_and_comments(intervals: List[Dict[str, Any]]) -> None:
    """
...
def extract_data(infotree: Infotree, source_code: str) -> List[Dict[str, Any]]:
    """
    ...
    source_code = split_snippet(source_code)[1]
